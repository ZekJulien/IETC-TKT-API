...
public static class TicketRouter
{
    public static WebApplication AddTicketRouter(this WebApplication app)
    {
        ...
        group.MapGet("stats", async (ClaimsPrincipal user, IGetTicketStatsUseCase useCase) =>
        {
            var stats = await useCase.ExecuteAsync(new GetTicketStatsInput(user.GetCompanyId(), user.GetAccountId()));
